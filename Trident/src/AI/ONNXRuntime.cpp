...
namespace Trident
{
    namespace AI
    {
        ONNXRuntime::ONNXRuntime() : m_EnableDirectML(true), m_EnableCPUFallback(true), m_IsDirectMLActive(false), m_IsCPUActive(false),
        ...
        {
            m_EnableDirectML = enableDirectML;
        }
        ...
        bool ONNXRuntime::LoadModel(const std::string& modelPath)
        {
            ...
            if (!m_IsDirectMLActive && !m_IsCPUActive)
            {
            ...
            try
            {
                ...
                TR_CORE_INFO("ONNX Runtime: Model '{}' loaded with DirectML={} CPUFallback={}", modelPath, m_IsDirectMLActive, m_IsCPUActive);
        ...
        void ONNXRuntime::ConfigureExecutionProviders()
        {
            ...
            {
#if defined(_MSC_VER)
                OrtStatus* l_Status = OrtSessionOptionsAppendExecutionProvider_DML(m_SessionOptions, 0);
                if (l_Status == nullptr)
                {
                    m_IsDirectMLActive = true;
                    TR_CORE_INFO("ONNX Runtime: DirectML execution provider appended.");
                }
                else
                {
                    TR_CORE_WARN("ONNX Runtime: DirectML provider unavailable - {}", Ort::GetApi().GetErrorMessage(l_Status));
                    Ort::GetApi().ReleaseStatus(l_Status);
                }
#else
                TR_CORE_WARN("ONNX Runtime: DirectML requested but compiler lacks MSVC extensions required to load it.");
#endif
            }
#endif

            if (m_EnableCPUFallback || !m_IsDirectMLActive)
            {
