...
{
    /**
     * @brief Gameplay-focused camera mirroring the editor implementation.
     *
     * The runtime camera intentionally mirrors the editor camera so transitions
     * between play and edit modes remain seamless. Gameplay systems can drive
     * this camera while the editor keeps its own instance, allowing the renderer
     * to route per-viewport requests without cross-contamination. Comments call
     * out the points where future multi-camera expansion can hook additional
     * state (shake, post processing, etc.).
     */
    ...
    private:
        glm::vec3 m_Position{ 0.0f, 1.8f, 6.0f };   ///< Gameplay default roughly matching a standing character height.
        glm::vec3 m_Rotation{ 0.0f };               ///< Euler rotation in degrees for simple scripting.
